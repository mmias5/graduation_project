...
/* Right-side actions + Edit button */
...
/* 🔹 New: right-side actions container + Edit button */
.hero-actions{ display:flex; align-items:center; gap:10px; }
.edit-btn{
  display:inline-block; padding:10px 16px; border-radius:999px;
  font-weight:800; font-size:14px; text-decoration:none;
...
.join{
  ...
  padding:14px 28px; line-height:1; font-size:18px; font-weight:900;
  border:none; border-radius:999px;
  background:#f4df6d; color:#2b2f55;
  box-shadow:0 12px 26px rgba(255,213,1,.35); cursor:pointer;
  margin:26px 0 0; text-align:center;
}
.join:hover{
  background:#fff; color:#fae06e; box-shadow:0 12px 26px rgba(141,141,141,.35);
}

/* ========= Simple Modal ========= */
.modal-wrap{ position:fixed; inset:0; display:flex; align-items:center; justify-content:center;
...
.modal{ width:100%; max-width:420px; background:#fff; border-radius:20px; box-shadow:var(--shadow);
  border:1px solid #e8eaf3; overflow:hidden; }
.modal-header{ display:flex; align-items:center; gap:12px; padding:18px 18px 0 18px; }
.icon-badge{ width:44px; height:44px; border-radius:50%; display:grid; place-items:center; background:#ecfdf5; border:2px solid #bbf7d0; }
...
.btn{ appearance:none; border:0; padding:10px 14px; border-radius:12px; font-weight:800; cursor:pointer; }
    ...
    <div class="wrap">
      <div class="hero-card" style="--hero-bg: url('tools/pics/social_life.png');">
        ...
        <div class="hero-top">
          ...
          <!-- 🔹 New top-right area: tag + Edit button -->
          <div class="hero-actions">
            <div class="tag">1st in clubs</div>

            <!-- Show to everyone; or wrap with a PHP check for leader -->
  ...
  <script>
    (function(){
      ...
      joinBtn.addEventListener('click', () => {
        ...
        if (alreadySent) { setSecondState(); } else { setFirstState(); localStorage.setItem(JOIN_KEY, '1'); }
        openModal();
      });
